...
* -------------------
* 
* file_name: The name of the file that is being assembled, used as the output file name with .hack
*            appended to the end.
* instructions: The array of instructions to translate to Hack machine code.
* num_instructions: The number of instructions in the instructions array.
*/
void assemble(const char* file_name, instruction* instructions, int num_instructions)
{
    char* fout_name = (char*)malloc(strlen(file_name) + 5);
    strcpy(fout_name, file_name);
    strcat(fout_name, ".hack");
    ...
    for (int i = 0; i < num_instructions; i++)
    {
        ...
        if (instr.itype == INSTR_A)
        {
            ...
            else
            {
                ...
                if (symbol == NULL)
                {
                    symtable_insert(instr.instr.a.operand.label, variable_address);
                    op = variable_address++;
                }
                else
                {
...
* --------------------------------
*
* instr: The instruction to translate to an opcode.
*
* returns: The opcode the instruction was translated into.
*/
